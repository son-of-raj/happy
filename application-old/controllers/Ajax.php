...
// Continuing if we got a result
    ...
    public function add_user_currency() {
        ...
        if (!empty($currrency)) {
            ...
            if (!empty($wallet_history) && count($wallet_history) > 0) {
    ...
    public function user_select_city()
    {
        $inp = $this->input->post();
        if ($inp['city']!='') 
        {
            ...
            //map lat long
            $formattedAddr = str_replace(' ','+',$inp['city']);
            $url = "https://maps.googleapis.com/maps/api/geocode/json?address=".$formattedAddr."&sensor=false&key=".$map_key;
            $details = file_get_contents($url);
            $det = json_decode($details,true);
            $formatted_address = $det['results'][0]['formatted_address'];
            $lat = $det['results'][0]['geometry']['location']['lat'];
            $long = $det['results'][0]['geometry']['location']['lng'];
            $this->session->set_userdata('selected_city', $inp['city']);
            $this->session->set_userdata('current_location', $formatted_address);
        ...
        {
            $this->session->unset_userdata('selected_city');
            $this->session->unset_userdata('current_location');
