...
<div class="page-wrapper">
	<div class="content container-fluid">
		...
		<div class="page-header">
			<div class="row">
				...
				<div class="col-auto text-end">
					...
					<a href="<?php echo base_url().'users/add'; ?>" class="btn btn-white add-button"><i class="fas fa-plus"></i></a>
				</div>
			</div>
		</div>
		<!-- /Page Header -->
		...
		<!-- Search Filter -->
		<form action="<?php echo base_url()?>users-list" method="post" id="filter_inputs">
                                ...
                                <tbody>
									...
									if(!empty($lists)) {
										...
										if(!empty($rows['created_at'])){
											$createdate=date(settingValue('date_format'), strtotime($rows['created_at']));
								  		}else{
											$createdate='-';
										}

										if(!empty($rows['last_login'])){
											$lastdate=date(settingValue('date_format'), strtotime($rows['last_login']));
								  		}else{
											$lastdate='-';
										}
										if(!empty($rows['covid_vaccine_txt'])){
											$covid_vaccine_txt = $rows['covid_vaccine_txt'];
										}else{
											$covid_vaccine_txt = '';
										}
										...
										<td>'.$createdate.'</td>
										<td>'.$lastdate.'</td>
										<td>'.$covid_vaccine_txt.'</td>
                                        <td><div '.$tag.'>
												<div class="status-toggle mb-2">
													<input  disabled id="status_'.$rows['id'].'" class="check change_Status_user1" data-id="'.$rows['id'].' type="checkbox" '.$val.'>
													<label for="status_'.$rows['id'].'" class="checktoggle">checkbox</label>
												</div>
											</div>
											</td>
										<td>
											<a href="javascript:;" class="on-default remove-row btn btn-sm bg-danger-light me-2 delete_user_data delete_show" id="Onremove_'.$rows['id'].'" data-id="'.$rows['id'].'"><i class="far fa-trash-alt me-1"></i> Delete</a>
											</td>
										</tr>';
										}
                                    }
