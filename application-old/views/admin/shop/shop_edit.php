...
$shop = $this->db->get_where('shops',array('id'=>$shop_id))->row_array();
$pname = $this->db->select('name')->where('id',$shop['provider_id'])->get('providers')->row()->name;
$cname = $this->db->select('category_name')->where('id',$shop['category'])->get('categories')->row()->category_name;
$sname = $this->db->select('subcategory_name')->where('id',$shop['subcategory'])->get('subcategories')->row()->subcategory_name;
?>
		...
		<div class="row">
			<div class="col-xl-8 offset-xl-2">
				...
				<div class="page-header">
					<div class="row">
						<div class="col">
							<h3 class="page-title">Edit Shop</h3>
						</div>				
					</div>
				</div>
				<!-- /Page Header -->
				<div class="card">
					<div class="card-body">
						<form id="shop_edit" method="post" autocomplete="off" enctype="multipart/form-data">
							<input type="hidden" class="form-control"  name="user_id" value="<?php echo (!empty($shop['id']))?$shop['id']:''?>" id="user_id">
							
							<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>"/>
							<div class="row" >
								<div class="form-group col-xl-4 float-start">
									<p class="text-muted">Provider Name</p>
									<label class="text-info"><?php echo $pname?></label>									
								</div>
								<div class="form-group col-xl-4 float-start">
									<p class="text-muted">Shop Category</p>
									<label class="text-info"><?php echo $cname?></label>									
								</div>
								
								<div class="form-group col-xl-4 float-start">
									<p class="text-muted">Shop Sub-Category</p>
									<label class="text-info"><?php echo $sname?></label>									
								</div>								
							</div>
							<div class="form-group">
								<label>Shop Name</label>
								<input class="form-control" type="text"  name="shop_name" id="shop_name" value="<?php echo (!empty($shop['shop_name']))?$shop['shop_name']:''?>">
							</div>		
                            <div class="form-group">
                                <?php $mob_no = '+'.$shop['country_code'].$shop['contact_no']; ?>

                                <label>Mobile Number</label><br>
                                <input type="hidden" name="country_code" id="country_code" value="<?php echo (!empty($shop['country_code']))?$shop['country_code']:''?>">
                                <input class="form-control no_only umobileno" type="text" name="contact_no" id="mobileno" value="<?php echo (!empty($shop['contact_no']))?$mob_no:''?>">
                            </div>					
							
							<div class="form-group">
								<label>Email</label>
								<input class="form-control" type="text"  name="email" id="email" value="<?php echo (!empty($shop['email']))?$shop['email']:''?>">
							</div>
            ...
            $availability_details = (!empty($shop['availability']))?$shop['availability']:'';
            ...
            if(!empty($shop['all_days'])){
                $checked = ($shop['all_days']=='1')?'checked':'';
            }else{
                $checked='';
            }
            ...
            <tr>
            <td>
            <input type="checkbox" class="err_check days_check me-1 validate_time" data-id="0" name="availability[0][day]" value="1" <?php echo $checked; ?>>All Days
            </td>
            <td class="">
            From Time <span class="time-select mb-1">
            <select class="form-control daysfromtime_check"  name="availability[0][from_time]" required>
...
<option value="<?php echo $sun_to_time?>"><?php echo $sun_to_time?></option>
            ...
            ?>
							...
							<?php }?>
								...
								<a href="<?php echo $base_url; ?>shop-lists"  class="btn btn-danger">Back</a>
							</div>
						</form>
					</div>
				</div>
