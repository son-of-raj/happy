						...
						<div class="table-responsive">
							<table class="table table-hover table-center mb-0 service_table" >
                                ...
                                <tbody>
									...
									$sessiontxt='';
									if($rows['autoschedule_session_no'] > 0) {
										$sessiontxt = "<br><small><i> session(".$rows['autoschedule_session_no'].")</i></small>";
									}
									echo '<tr>
										<td>'.$i++.'</td>
										<td>'.$service_date.'</td>
										...
										<td>'.$rows['service_title'].$sessiontxt.'</td>
										...
										if($rows['admin_change_status']==1){
											...
											<a href="#" class="btn btn-sm bg-warning-light me-2"><i class="fa fa-check me-1"></i> Changed</a>
										</td>'; 
										}else{
											if ($rows['cod']!=1) {
												if($rows['amount'] > 0){
													echo'<td>
														<a href="'.base_url().'reject-payment/'.$rows['id'].'" class="btn btn-sm bg-success-light me-2"><i class="far fa-edit me-1"></i> Edit</a>
													</td>';
												} else {
													echo'<td>-</td>';
												}
