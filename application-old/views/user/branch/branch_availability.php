<?php
    ...
    if(!empty($branch_id)){
		$provider_availability = $this->db->where('provider_id',$user_id)->where('id',$branch_id)->get('branch')->row_array();
	}else{
...
<option value="<?php echo $fri_from_time?>"><?php echo $fri_from_time?></option>
            ...
            <td class="">
            <?php echo (!empty($user_language[$user_selected]['lg_To_time'])) ? $user_language[$user_selected]['lg_To_time'] : $default_language['en']['lg_To_time']; ?> <span class="time-select" >
            <select class="form-control  custom-height eachdaytotime eachdaytotime5"  data-id="5" name="availability[5][to_time]" <?php echo $day_name[5]; ?>>
                                                  <?php
