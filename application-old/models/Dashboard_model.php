...
class Dashboard_model extends CI_Model {
	...
	public function __construct() {
		...
		private function p_get_datatables_query()
		{
			...
			$this->db->select('U.id,U.name,U.profile_img,U.mobileno,U.email,created_at as created_at,U.last_login,U.status,U.type,U.covid_vaccine,S.subscription_name,SD.subscriber_id,country_code');
    ...
    public function get_users_device($city_id) {

    	$this->db->select("device_details.user_id, device_details.device_id");
        
        $this->db->join('users', 'device_details.user_id = users.id', 'LEFT');
        if ($city_id!='') {
        	$this->db->join('user_address', 'device_details.user_id = user_address.user_id', 'LEFT');
        	$this->db->where('city_id',$city_id);
        }
        $this->db->where(['users.status'=>1, 'users.is_agree'=>1, 'device_details.device_type'=>'android', 'device_details.type'=>'2']);
        $android = $this->db->get('device_details')->result_array();
        ...
        $this->db->where(['users.status'=>1, 'users.is_agree'=>1, 'device_details.device_type'=>'ios', 'device_details.type'=>'2']);
        $ios = $this->db->get('device_details')->result_array();

        return ['android'=>$android, 'ios'=>$ios];
    }

    public function get_provider_device($city_id, $user_type, $subscription_id) {

    	$this->db->select("device_details.user_id, device_details.device_id");
        
        $this->db->join('providers', 'device_details.user_id = providers.id', 'LEFT');
        if ($city_id!='') {
        	$this->db->join('provider_address', 'device_details.user_id = provider_address.provider_id', 'LEFT');
        	$this->db->where('provider_address.city_id',$city_id);
        }
        if ($subscription_id!='') {
        	$this->db->join('subscription_details', 'device_details.user_id = subscription_details.subscriber_id and type=1', 'LEFT');
        	$this->db->where('subscription_details.subscription_id',$subscription_id);
        }
        $this->db->where(['providers.status'=>1, 'providers.is_agree'=>1, 'device_details.device_type'=>'android', 'device_details.type'=>'1', 'providers.type'=>$user_type]);
        $android = $this->db->get('device_details')->result_array();
        ...
        $this->db->where(['providers.status'=>1, 'providers.is_agree'=>1, 'device_details.device_type'=>'ios', 'device_details.type'=>'1', 'providers.type'=>$user_type]);
        $ios = $this->db->get('device_details')->result_array();

        return ['android'=>$android, 'ios'=>$ios];
    }

    public function get_freelancers_device($city_id, $user_type) {
