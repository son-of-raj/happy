...
$pname = $this->db->select('name')->where('id',$user['provider_id'])->get('providers')->row()->name;

$cname = '';
$sname = '';
if(!empty($user['category'])){
	$cname = $this->db->select('category_name')->where('id',$user['category'])->get('categories')->row()->category_name;
}
if(!empty($user['subcategory'])){
	$sname = $this->db->select('subcategory_name')->where('id',$user['subcategory'])->get('subcategories')->row()->subcategory_name;
}
?>
	...
	<div class="content container-fluid">
		<div class="row">
			<div class="col-xl-8 offset-xl-2">
				...
				<div class="card">
					<div class="card-body">
						<form id="staff_edit" method="post" autocomplete="off" enctype="multipart/form-data">
							<input type="hidden" class="form-control"  name="user_id" value="<?php echo (!empty($user['id']))?$user['id']:''?>" id="user_id">
							
							<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>"/>
							
							<div class="row" >
								<div class="form-group col-xl-4 float-left">
									<p class="text-muted">Staff Category</p>
									<label class="text-info"><?php echo $cname?></label>									
								</div>
								
								<div class="form-group col-xl-4 float-left">
									<p class="text-muted">Staff Sub-Category</p>
							...
							<div class="form-group">
								...
								<input class="form-control" type="text"  name="first_name" id="first_name" value="<?php echo (!empty($user['first_name']))?$user['first_name']:''?>">
							</div>
							<div class="form-group d-none">
								<label>Last Name</label>
								<input class="form-control" type="text"  name="last_name" id="last_name" value="<?php echo (!empty($user['last_name']))?$user['last_name']:' '?>">
							</div>
                            <div class="form-group">
                                <?php $mob_no = '+'.$user['country_code'].$user['contact_no']; ?>

                                <label>Mobile Number</label><br>
                                <input type="hidden" name="country_code" id="country_code" value="<?php echo (!empty($user['country_code']))?$user['country_code']:''?>">
                                <input class="form-control no_only umobileno" type="text" name="contact_no" id="mobileno" value="<?php echo (!empty($user['contact_no']))?$mob_no:''?>">
                            </div>      
							\
							<div class="form-group">
								<label>Email</label>
            ...
            $availability_details = (!empty($user['availability']))?$user['availability']:'';
            ...
            if(!empty($user['all_days'])){
                $checked = ($user['all_days']=='1')?'checked':'';
            }else{
                $checked='';
            }
...
<option value="<?php echo $fri_from_time?>"><?php echo $fri_from_time?></option>
            ...
            <td class="">
            To Time <span class="time-select" >
            <select class="form-control  custom-height eachdaytotime eachdaytotime5 select" name="availability[5][to_time]" <?php echo $day_name[5]; ?>>
                                                  <?php
...
<option value="<?php echo $sun_to_time?>"><?php echo $sun_to_time?></option>
            ...
            ?>
							...
							<?php }?>
								...
								<a href="<?php echo $base_url; ?>staff-lists"  class="btn btn-danger">Cancel</a>
							</div>
						</form>
					</div>
				</div>
