<div class="page-wrapper">
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row">
                ...
                <div class="col-auto text-end">
		            <a href="<?php echo $base_url; ?>add-language" class="btn btn-white add-button"><i class="fas fa-plus"></i></a>
		        </div>
            </div>
        </div>
        ...
        <ul class="nav nav-tabs menu-tabs">
            ...
            <li class="nav-item ">
                <a class="nav-link" href="<?php echo $base_url; ?>wep-language">Web Keywords</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="<?php echo $base_url; ?>app-page-list">App Keywords</a>
            </li>
        </ul>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        if ($row['language_value'] == 'en') {
                                                            ...
                                                            <a href="javascript:void(0);" id="lang_status<?php echo $row['id']; ?>" data-status="<?php echo $row['status']; ?>" onclick="change_status(<?php echo $row['id']; ?>)" class="badge badge-<?php echo $status; ?>"><span id="texts<?php echo $row['id']; ?>"><?php echo $stst; ?></span></a>
													...
													<td><a href="javascript:;" class="on-default remove-row btn btn-sm bg-danger-light me-2" onclick="lang_delete('<?php echo $row['language_value']; ?>')" ><i class="far fa-trash-alt me-1"></i> Delete</a></td>
                                                </tr>
			...
			$.ajax({
				...
				success: function (response)
				{
					...
					if (response == 'success')
					{
						...
						$('#flash_lang_message').html(" <div class='alert alert-danger fade in' id=''>Something went wrong. <button type='button' class='close' data-bs-dismiss='alert' aria-label='Close'><span aria-hidden='true'>&times;</span></button></div>");
					}
				}
			});
...
function change_status(id)
{
	...
	$.ajax({
		...
		success: function (response)
		{
			...
			if (response == 0)
			{
				$('#flash_lang_message').html(" <div class='alert alert-danger fade in' id=''>Default Language cannot be inactive. <button type='button' class='close' data-bs-dismiss='alert' aria-label='Close'><span aria-hidden='true'>&times;</span></button></div>");
