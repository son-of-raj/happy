...
<?php
$booking_count = $this->service->booking_count_by_manager($this->session->userdata('id'));
$services_count = $this->service->services_count_by_manager($this->session->userdata('id'));
...
<div class="content">
	<div class="container">
		<div class="row">
			<?php $this->load->view('user/home/manager_sidemenu'); ?>

			<div class="col-xl-9 col-md-8">
				<div class="row">
					<div class="col-lg-4">
						<a href="<?php echo base_url() ?>manager-bookings" class="card">
							<div class="card-body">
								<div class="row">
									<div class="col-6">
										<div class="avatar">
								...
								<div class="card-body">
									...
									<ul class="row">
										<li class="col-6 col-lg-6">
											<p><?php echo (!empty($user_language[$user_selected]['lg_started_on'])) ? $user_language[$user_selected]['lg_started_on'] : $default_language['en']['lg_started_on']; ?> <?php if (!empty($my_subscribe['subscription_date'])) {
										...
										?>
										<li class="col-6 col-lg-6">
											<p><?php echo (!empty($user_language[$user_selected]['lg_price'])) ? $user_language[$user_selected]['lg_price'] : $default_language['en']['lg_price']; ?> <?php if (!empty($subscription_name['fee'])) {
																																																			echo currency_conversion($user_currency_code) . get_gigs_currency($subscription_name['fee'], $subscription_name['currency_code'], $user_currency_code);
																																																		}
																																																		?></p>
										</li>
									</ul>
									...
									<ul class="row">
										<li class="col-sm-6">
											<p><?php echo (!empty($user_language[$user_selected]['lg_paid_at'])) ? $user_language[$user_selected]['lg_paid_at'] : $default_language['en']['lg_paid_at']; ?> <?php if (!empty($my_subscribe['subscription_date'])) {
																																																				echo date('d M Y', strtotime($my_subscribe['subscription_date']));
																																																			} ?></p>
										</li>
										<li class="col-sm-6">
											<p><span class="text-success"><?php echo (!empty($user_language[$user_selected]['lg_paid'])) ? $user_language[$user_selected]['lg_paid'] : $default_language['en']['lg_paid']; ?></span> <span class="amount"><?php if (!empty($subscription_name['fee'])) {
																																																																echo currency_conversion($user_currency_code) . get_gigs_currency($subscription_name['fee'], $subscription_name['currency_code'], $user_currency_code);
																																																															} ?></span></p>
										</li>
									</ul>
							...
							<div class="col-lg-4">
								<div class="sp-plan-action card-body">
									...
									<div class="next-billing">
										<p><?php echo (!empty($user_language[$user_selected]['lg_next_billing_on'])) ? $user_language[$user_selected]['lg_next_billing_on'] : $default_language['en']['lg_next_billing_on']; ?> <span><?php if (!empty($my_subscribe['subscription_date'])) {
