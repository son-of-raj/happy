...
class Chat_model extends CI_Model
{
  ...
  /* New Code */
  public function get_chat_list_provider(){
    ...
    foreach (array_unique($test) as $key => $value) {
        ...
        $token_detail[]=$this->get_providertoken_inform($value);
  ...
  public function get_providertoken_inform($token){        
	...
	public function get_chat_list_user($token){
    ...
    foreach (array_unique($test) as $key => $value) {
        ...
        $token_detail[]=$this->get_usertoken_inform($value);
        
    }
    return $token_detail;

  }
	public function get_usertoken_inform($token){
	...
	public function get_conversation_inform($user_token,$partner_token){
        $return=$this->db->select('*')->
                from('chat_table')->
                where("(`sender_token` = '".$user_token."' AND `receiver_token` = '".$partner_token."') OR (`sender_token` = '".$partner_token."' AND `receiver_token` = '".$user_token."')")->
                where('status',1)->
                group_by('chat_id')->
