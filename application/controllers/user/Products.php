...
class Products extends CI_Controller
{
    ...
    public function product_buy_now()
    {
        ...
        if (!empty($cartlist)) {
            ...
            $order_data['total_qty'] = array_sum(array_map(function ($item) {
                return $item['qty'];
            }, $cartlist));
            $order_data['total_amt'] = round(array_sum(array_map(function ($item) {
                return $item['product_total'];
            }, $cartlist)), 2);
    ...
    public function razorpay_order_payment()
    {
        $user_inp = $this->input->post();
        $userId = $this->session->userdata('id');
        ...
        $this->order_success($user_inp['order_id'], 'razorpay', $user_inp['token'], 'success', $user_inp['address_type']);
        echo json_encode('success');
    }
