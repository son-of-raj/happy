(function ($) {
  ...
  $(document).ready(function () {
    ...
    $("#service_subcategory").on("change", function () {
      ...
      $("#service_sub_subcategories").html(
        '<option value="">Select subcategory</option>'
      );
      if ($(this).val() != "") {
        $.ajax({
    ...
    //     var date = dateStr.split(" to ")[0]; // For the start date of the selected range
    ...
    $("#reg_user")
      ...
      .on("success.form.bv", function (e) {
        ...
        var country_code = $("#user_mobile").intlTelInput(
          "getSelectedCountryData"
        ).dialCode;
        $.ajax({
          type: "POST",
  ...
  $(".err_check").on("change", function () {
    ...
    if (!this.checked) {
      $(".eachdayfromtime" + ids).val("");
      $(".eachdayfromtime" + ids).removeAttr("style");
      $(".eachdaytotime" + ids).val("");
      $(".eachdaytotime" + ids).removeAttr("style");
    }
    if (alldays > 0) {
    }
    ...
    if ($(".days_check").prop("checked") == true) {
      ...
      $(".eachdays").each(function () {
        ...
        if ($(".eachdaytotime" + row).val() != "") {
          ...
          if (
            $(".eachdays" + row).prop("checked") == false ||
            from_time == ""
          ) {
            $(".eachdayfromtime" + row).attr("style", "border-color:red");
            $(".eachdaytotime" + row).removeAttr("style");
  ...
  function export_invoice() {
    ...
    if (from_date != "" && to_date != "") {
      window.open(
        base_url +
          "user/dashboard/export_muliple_invoice/" +
          from_date +
          "/" +
          to_date,
        "_blank"
  ...
  function submitDetailsForm() {
    ...
    if (n == 0) {
      ...
      if (daycount != 0) {
        ...
        $(".eachdaytotime").attr("style", "border-color:red");
        $(".available_title").text(
          "Please Select Day and Relevant From & To Time"
        );
        return false;
      }
