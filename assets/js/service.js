(function ($) {
  ...
  $(document).ready(function () {
    ...
    $("#submit_reward").on("click", function () {
      ...
      if (r == 0) {
        $.ajax({
          ...
          data: {
            reward_id: $("#h_reward_id").val(),
            user_id: $("#h_user_id").val(),
            service_id: $("#r_service_id").val(),
            reward_type: $("#reward_type").val(),
            reward_discount: $("#reward_discount").val(),
            total_visit_count: $("#h_total_visit_count").val(),
    ...
    $("#submit_offer").on("click", function () {
      ...
      if (so == 0) {
        $.ajax({
          ...
          data: {
            service_id: $("#hservice_id").val(),
            offer_percentage: $("#offer_percentage").val(),
    ...
    $("#m_submit_offer").on("click", function () {
      ...
      if (mo == 0) {
        $.ajax({
          ...
          data: {
            sids: $("#hsids").val(),
            offer_percentage: $("#m_offer_percentage").val(),
            start_date: $("#m_start_date").val(),
            end_date: $("#m_end_date").val(),
            start_time: $("#m_start_time").val(),
            end_time: $("#m_end_time").val(),
            csrf_token_name: csrf_token,
          },
          success: function (data) {
            window.location.reload();
    ...
    $(".cpntype").on("change", function () {
      ...
      if (ctype == 2) {
        $("#coupon_amt").show();
        $("#coupon_price").addClass("inputcls");
        $("#coupon_per").hide();
        $("#coupon_percent").removeClass("inputcls");
      } else {
        $("#coupon_per").show();
        $("#coupon_percent").addClass("inputcls");
        $("#coupon_amt").hide();
        $("#coupon_price").removeClass("inputcls");
      }
    });
    ...
    $(".viewServiceCoupon").on("click", function () {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          $("#view_name").text("PRO" + obj.name);
          $("#view_type").text(vtype);
          $("#view_per").text(obj.percentage);
          $("#view_amt").text(obj.price);
          $("#view_from").text(obj.start_date);
          $("#view_end").text(obj.end_date);
          $("#view_desc").text(obj.description);
          $("#view_until").text(obj.valid_days + " days");
          $("#view_user").text(obj.user_limit + atxt);
          $("#view_num").text(obj.user_count);
          $("#view_statu").text(obj.user_status);
          $("#view_sname").text(title);
        ...
        fields: {
          ...
          service_amount: {
            validators: {
              digits: {
                message:
                  "Please Enter valid service amount and not used in special characters...",
              },
              notEmpty: {
          ...
          booking_amount: {
            validators: {
              digits: {
                message: "Please Enter valid booking amount",
              },
              notEmpty: {
      ...
      .on("success.form.bv", function (e) {
        ...
        if (divCount > 0) {
          ...
          if (n == 0) {
            ...
            $("html, body").animate(
              { scrollTop: jQuery("#addiservice_div").offset().top },
              0
            );
            return false;
          }
        ...
        validators: {
          ...
          duration: {
            validators: {
              digits: {
                message:
                  "Please Enter valid service duration and use only numbers...",
              },
              notEmpty: {
    ...
    function RenderThumbnail(e, readerEvt) {
      ...
      li.innerHTML = [
        '<div class=" upload-images"> ' +
          '<a style="display:block;" href="javascript:void(0);" class="file_close btn btn-icon btn-danger btn-sm">X</a><img class="thumb" src="',
        e.target.result,
        '" title="',
