(function ($) {
  ...
  $("#book_services")
    ...
    .on("success.form.bv", function (e) {
      ...
      if (d > estTime) {
        ...
        final_gig_amount1 = $("#booking_amount").val();
        ...
        if (issave == 0) {
          $.ajax({
            ...
            data: {
              ...
              final_amount: final_gig_amount1,
  ...
  function load_staff_availability(
    date,
    provider_id,
    service_id,
    staff_id,
  ...
  $(document).on("change", ".guestservice", function () {
    ...
    var val = $(this).val();
    if (val != "") {
      var amt = $("#guest_ser" + id + ' option[value="' + val + '"]').attr(
        "data-seramount"
      );
      var dur = $("#guest_ser" + id + ' option[value="' + val + '"]').attr(
        "data-serduration"
      );
      var sign = $("#guest_ser" + id + ' option[value="' + val + '"]').attr(
        "data-currency_sign"
      );
  ...
  $("#submit_button_id").on("click", function () {
    ...
    if (d > estTime) {
      ...
      var e = 0;
      if (
        $("input[type='checkbox'][name='service_offered_guest[]']:checked")
          .length > 0
      ) {
            ...
            if (status == 1) {
              swal({
                ...
                window.location.href =
                  base_url + "service-checkout/" + response.url;
              });
            } else {
              swal({
                title: response.title,
...
$(document).ready(function () {
  $('#staff_id option[data-homeservice="1"]')
    .hide()
    .parent()
...
$(".serviceat").on("change", function () {
  ...
  if (val == 1) {
    ...
    $('#staff_id option[data-homeservice="0"]')
      .hide()
      .parent()
      .selectpicker("refresh");
    $('#staff_id option[data-homeservice="1"]')
...
$("#staff_id").on("change", function () {
  ...
  var selarea = $('#staff_id option[value="' + val + '"]').attr(
    "data-homeservicearea"
  );
...
function initMap() {
  ...
  $.get(
    ...
    { address: address, key: key, language: language_option },
...
function load_msp_details(latitude, longitude) {
  ...
  var map = new google.maps.Map(document.getElementById("map"), {
    ...
    style: {
      background:
        "url('http://ressio.github.io/lazy-load-xt/dist/loading.gif') center center no-repeat",
    },
  });
...
function geolocate() {
  ...
  $.get(
    "https://maps.googleapis.com/maps/api/geocode/json",
    { latlng: latLng, key: key, language: language_option },
    function (data, status) {
